...
class OptimizedCallProcessor:
    def __init__(self):
        ...
        # Processing delays optimized for dev tier (300 RPM per key)
        self.download_delay = 0.1  # Minimal delay for RingCentral
        self.min_transcription_interval = 0.0  # Let rate limiter handle the timing
    ...
    def get_fresh_recording_uri(self, recording_id: str) -> Optional[str]:
        ...
        try:
            ...
            if response.ok():
                ...
                logger.error(f"Failed to refresh recording URI")
                return None
        except Exception as e:
            ...
            try:
                ...
                if response.ok():
                    ...
                    if 'navigation' in data and 'nextPage' in data['navigation']:
                        ...
                        break
                elif hasattr(response, 'response') and response.response.status_code == 401:
                    logger.warning("Token expired during call log fetch, refreshing...")
                    self._authenticate()
                    continue
                else:
                    logger.error(f"Failed to fetch call logs")
                    break
